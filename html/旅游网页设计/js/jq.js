...
$(function(){
    ...
    $.each(showlist,function(){
        var t_right=$(this).find('.t_right span');//获得当前大分类下的每一个导航
        var detail_list=$(this).find('.content_detail .cd_right');//获得当前大分类下的小分类内容
        t_right.hover(function(){
                t_right.removeClass('tr-hover');
                $(this).addClass('tr-hover');
                var k=t_right.index($(this));//获取当前下标值
                detail_list.removeClass('cr-hover');
                detail_list.eq(k).addClass('cr-hover');
            },
            function(){});

        /*鼠标滑动小图背景*/
        var rl_img=detail_list.find('.rl_img');//获取该分类下的所有6个小图
        rl_img.hover(function(){
                var a=$(this).find('.rl_bg');
                a.animate({top:-125},'fast');
            },
            function(){
                var a=$(this).find('.rl_bg');
                a.animate({top:125},'fast');
