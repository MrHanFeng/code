...
function openViewer(basePath,pressCode) {
	var testObj = document.getElementById("__acrobat_test_id");
	
//	if( testObj ) {
//		if( window.confirm("您尚未安装PDF文档阅读器，是否下载安装？") )
//			window.open(basePath+"/media/resource.do?method=download&seqNo=1","_blank");
//	}
//	else {
//		var urlString = basePath+"/media/cataSample.do?method=openViewer&pressCode="+pressCode;
//		var statusString = "top=0,left=0,directories=no,toolbar=no,menubar=no,scrollbars=no,resizable=yes,locationbar=false,status=no,fullscreen=yes";
//		window.open(urlString,"_blank",statusString);    
//	}
	var urlString = basePath+"/media/cataSample.do?method=openViewer&pressCode="+pressCode;
	var statusString = "top=0,left=0,directories=no,toolbar=no,menubar=no,scrollbars=no,resizable=yes,locationbar=false,status=no,fullscreen=yes";
	window.open(urlString,"_blank",statusString);
	...
	if( key != null )
	{
		if( !removeIcon )
		{
			if (visible)
			{
		 		...
		 		key.innerHTML="<img src='"+imgPath+"/plus.gif' width='16' height='16' hspace='0' vspace='0' border='0'>";
			}
			else
			{
		  		div.style.display="block";
				key.innerHTML="<img src='"+imgPath+"/minus.gif' width='16' height='16' hspace='0' vspace='0' border='0'>";
			}
		}
		else
			key.innerHTML="<img src='"+imgPath+"/transparent.gif' width='16' height='16' hspace='0' vspace='0' border='0'>";
	}
...
function Expand() {
   ...
   for (i=0;i<divs.length;i++) {
	 ...
	 key.innerHTML="<img src='"+imgPath+"/textfolder.gif' width='16' height='16' hspace='0' vspace='0' border='0'>";
   }
}
...
function Collapse() {
   ...
   for (i=0;i<divs.length;i++) {
	 ...
	 key=document.getElementById("x" + divs[i].id);
	 key.innerHTML="<img src='"+imgPath+"/folder.gif' width='16' height='16' hspace='0' vspace='0' border='0'>";
   }
}
...
function CreateTreeItem( parent, img1FileName, img2FileName, nodeName, url, target, leaf )
{
	...
	if( leaf==1 )	AddImage( a, imgPath+"/transparent.gif" );
	else AddImage( a, imgPath+"/plus.gif" );
    ...
    a.setAttribute( "title", nodeName );//add by h
	cell.appendChild( a );
	if( url != null )
	{
